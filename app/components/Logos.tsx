  ...
  return (
    <div className="border-b border-slate-600 bg-slate-800">
      <Wrapper className="border-slate-600">
        ...
        <div className="mt-6 grid grid-cols-2 border-t border-slate-600 sm:grid-cols-3 md:grid-cols-5">
          {Array.from({ length: LOGO_COUNT }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`flex h-24 items-center justify-center border-r border-slate-600 md:h-32 ${
                  lastRowXs ? "" : "border-b"
                } ${!lastRowSm ? "sm:border-b" : "sm:border-b-0"} ${!lastRowMd ? "md:border-b" : "md:border-b-0"}`}
              >
                <img src={LOGO_SRC} alt="Logo" className="h-5 w-auto brightness-0 invert md:h-6" />
              </div>
            );
          })}
        </div>
